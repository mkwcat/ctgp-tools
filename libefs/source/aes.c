/*
 *
 T his is an implementation of the AES alg*orithm, specifically ECB, CTR and CBC mode.
...
static void KeyExpansion(
    uint8_t* RoundKey, const uint8_t* Key
) {
    unsigned i, j, k;
...
void AES_init_ctx(
    struct AES_ctx* ctx, const uint8_t* key
) {
...
void AES_ctx_set_iv(
    struct AES_ctx* ctx, const uint8_t* iv
) {
    memcpy(ctx->Iv, iv, AES_BLOCKLEN);
}
...
static void AddRoundKey(
    uint8_t round, state_t* state, const uint8_t* RoundKey
) {
    uint8_t i, j;
    for (i = 0; i < 4; ++i) {
        for (j = 0; j < 4; ++j) {
...
static void Cipher(
    state_t* state, const uint8_t* RoundKey
) {
    uint8_t round = 0;
...
static void XorWithIv(
    uint8_t* buf, const uint8_t* Iv
) {
    uint8_t i;
