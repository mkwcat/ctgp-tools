...
u8 ios_dumper[] = {
	
};
...
int main(int argc, char **argv)
{
  ...
  net_deinit();
	...
	memcpy((void*) 0x93a740c0, replace_syscall, sizeof(replace_syscall));
	DCFlushRange((void*) 0x93a740c0, 0x400);

	if (ES_GetDeviceCert(0x80001000) != 0x12000000) {
		printf("ES_GetDeviceCert patch failed\n");
		while (1) { }
	}

	printf("done, make file...\n");
	saoPatchISFSAccess();
	ISFS_Initialize();
	s32 rest = ISFS_CreateFile("/title/lulz.bin", 0, 3, 3, 0);
	if (rest != 0) {
		if (rest == -105) {
			ISFS_Delete("/title/lulz.bin");
			ISFS_CreateFile("/title/lulz.bin", 0, 3, 3, 0);
		} else {
