...
int CopyFile(const char* src, const char* dst)
{
    ...
    if (!srcFile)
    {
        printf("diatool: cannot open source file\n");
        return 0;
    }

    FILE* dstFile = fopen(dst, "rb");
    if (dstFile)
    {
        fclose(srcFile);
        fclose(dstFile);
        // printf("diatool: destination file already exists\n");
        return 0;
    }
    ...
    {
        fclose(srcFile);
        printf("diatool: cannot open destination file\n");
        return 0;
    }
...
int main(int argc, char** argv)
{
    ...
    snprintf(realFileName, 256, "/Users/mkwcat/Documents/repo/ctgp-archive/update/%.8s%02X%02X%04X-%.8s%02X%02X%04X.dif",
    ...
    int rersult = CopyFile(argv[1], realFileName);
    if (rersult == 0) {
